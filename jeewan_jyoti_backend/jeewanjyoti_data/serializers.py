...
class Total_activity_DataSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['device_id','date','goal','distance','distance','step','exercise_time','calories','exercise_minutes','user']
...
class SleepDataSerializer(serializers.ModelSerializer): 
    class Meta:
        model = SleepData
        fields = ['device_id', 'date', 'start_time', 'end_time','duration', 'sleep_quality_sequence', 'awake_percentage', 'deep_sleep_percentage', 'light_sleep_percentage', 'medium_sleep_percentage','user']
